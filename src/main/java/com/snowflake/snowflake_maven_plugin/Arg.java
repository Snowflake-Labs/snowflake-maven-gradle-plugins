...
public class Arg {
  ...
  /**
   * This factory method is used to parse UDF/procedure arguments supplied on the command line e.g.
   * mvn snowflake-udx:deploy ... -Ddeploy.args="a string, b string" ... The input to this method
   * will be "a string, b string" And it will output and object array [Arg(a, string), Arg(b,
   * string)] We cannot make a new constructor or else Maven plugin will not be able to instantiate
   * args object
   */
  public static Arg[] fromString(String argsString) {
    ...
    for (String stringArg : stringArgs) {
      ...
      if (stringArgSplit.length != 2) {
        throw new IllegalArgumentException(
            String.format("Malformed argument: %s, from args: %s", stringArg, argsString));
      }
      Arg arg = new Arg();
