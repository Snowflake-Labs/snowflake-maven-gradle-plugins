    ...
    extends AbstractMojo
{
    ...
    /**
     * The functions to create. Encapsulates function name, handler, arg names, arg types, return types
     *
     * @parameter expression="${session}"
     * @required
     * @readonly
    ...
    /**
     * The procedures to create. Encapsulates function name, handler, arg names, arg types, return types
     *
     * @parameter expression="${session}"
     * @required
     * @readonly
    ...
    // Duplicate parameters from the auth configuration object so that users can specify auth arguments through CLI
    ...
    // Duplicate parameters from the functions/procedures configuration object so that users can specify a single function through CLI
    ...
    public void execute()
        ...
        ExecutionEnvironment executionEnv = new ExecutionEnvironment(mavenProject, mavenSession, pluginManager);
        ...
        udxs = Stream.concat(Arrays.stream(functions), Arrays.stream(procedures)).collect(Collectors.toList());
        ...
        getLog().info("Execute copy dependencies. Destination: " + buildDirectory + "/" + dependencyDirName);
        ...
        try {
            ...
            snowflake.uploadDependencies(String.format("%s/%s", buildDirectory, dependencyDirName), stage, depsToStagePath);
        } catch (SQLException e) {
    ...
    private void validateUserConfig() {
        ...
        if (compiler != null) {
            if (compiler.getConfiguration() != null) {
                ...
                if (compilerConfig.contains("<release>17</release>") || compilerConfig.contains("<target>17</target>")) {
                    getLog().warn(java17Warning);
                }
            }
        }
        Properties properties = mavenProject.getProperties();
        if (properties.getProperty("maven.compiler.release") != null && properties.getProperty("maven.compiler.release").contains("17")
                || properties.getProperty("maven.compiler.target") != null && properties.getProperty("maven.compiler.target").contains("17")) {
            getLog().warn(java17Warning);
        }
    }
        ...
        if (auth != null) {
            if (auth.containsKey(authFileParamName)) {
                ...
                try {
                    ...
                    throw new MojoExecutionException("Error reading or accessing the specified properties file: ", e);
                }
            }
    ...
    // Create a Function/Procedure object and append it to the UDFs list if ANY of the CLI arguments for a function/procedure are provided
    // May produce a Function/Procedure with null properties so that the user config validation can prompt the user for missing fields
    private void appendCliUdxIfDefined(List<UserDefined> udfs) {
        ...
        for (String udxCliParam : udxCliParams) {
            if (udxCliParam != null) {
                UserDefined udx = UserDefined.create(udx_type, udx_name, udx_args, udx_handler, udx_returns);
                udfs.add(udx);
                break;
            }
        }
    ...
    private void executeDependencyCopyDependencies(ExecutionEnvironment executionEnv) throws MojoExecutionException {
        executeMojo(
                plugin(
    ...
    private void executeDependencyList(ExecutionEnvironment executionEnv) throws MojoExecutionException {
        executeMojo(
                plugin(
                        groupId("org.apache.maven.plugins"),
                        artifactId("maven-dependency-plugin"),
    ...
    private Map<String, String> mapDependenciesToStagePaths() throws MojoExecutionException, IOException {
        ...
        try {
            ...
            throw new MojoExecutionException("Unexpected error: snowflake-udx plugin failed to create dependency list file");
        } finally {
            it.close();
        }
