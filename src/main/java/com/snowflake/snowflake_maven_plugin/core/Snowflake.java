...
/**
 * Class to execute create stage, upload file, create function/procedure commands on Snowflake through a JDBC connection
 */
public class Snowflake {
    ...
    public void uploadDependencies(String localFilePath, String stageName, Map<String, String> depsToStagePath) throws SQLException {
        ...
        for (Map.Entry<String, String> entry : depsToStagePath.entrySet()) {
            ...
            uploadFiles(String.format("%s/%s", localFilePath, dependencyFile), stageName, stagePath, false);
        }
        log.info("Dependency JARs uploaded!");
    }

    public void uploadFiles(String localFileName, String stageName, String pathOnStage, boolean overwrite) throws SQLException {
        Map<String, String> options = new HashMap<>();
        options.put("AUTO_COMPRESS", "FALSE");
        options.put("PARALLEL", "4");
        options.put("OVERWRITE", String.valueOf(overwrite));
        String sql = String.format("PUT %s %s %s",
    ...
    public String getImportString(String artifactFileName,  String stageName, Map<String, String> depsToStagePath) {
    ...
    public void createFunctionOrProc(UserDefined udx, String stageName, String fileName, Map<String, String> depsToStagePath) throws SQLException {
        String s = String.format("CREATE OR REPLACE %s %s (%s)\n", udx.getType(), udx.name, udx.getInputs())
                ...
                + String.format("IMPORTS = (%s);", getImportString(fileName, stageName, depsToStagePath)); // TODO: Store these variables as properties of the Snowflake class so that we avoid passing variables through several functions
