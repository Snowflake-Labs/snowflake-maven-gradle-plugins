...
public class SnowflakeTest {
  ...
  public void testUploadArtifact() throws SQLException {
    ...
    verify(statement, times(2))
        .execute(
            "PUT file://myproject.jar @mvn_stage/libs OVERWRITE = true AUTO_COMPRESS = FALSE PARALLEL = 4");
    sf.uploadArtifact("'file:///tmp/load data'", "@mvn_stage");
    verify(statement)
        .execute(
            "PUT 'file:///tmp/load data' @mvn_stage/libs OVERWRITE = true AUTO_COMPRESS = FALSE PARALLEL = 4");
  }
  ...
  public void testUploadDependencies() throws SQLException {
    ...
    verify(statement)
        .execute(
            "PUT file://target/dependency/dep.jar @mystage/com/google/dep/1.2.3/dep-1.2.3.jar OVERWRITE = false AUTO_COMPRESS = FALSE PARALLEL = 4");
    verify(statement)
        .execute(
            "PUT file://target/dependency/gson-2.10.jar @mystage/com/google/gson/2.10/gson-2.10.jar OVERWRITE = false AUTO_COMPRESS = FALSE PARALLEL = 4");
    verify(statement)
        .execute(
            "PUT file://target/dependency/gson-2.11.jar @mystage/com/google/gson/2.11/gson-2.11.jar OVERWRITE = false AUTO_COMPRESS = FALSE PARALLEL = 4");
  }
  ...
  public void testGetImportString() throws SQLException {
    ...
    assertEquals(
        "'@mystage/libs/project.jar', '@mystage/com/google/dep/1.2.3/dep-1.2.3.jar/dep.jar', "
